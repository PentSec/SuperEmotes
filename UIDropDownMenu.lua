...
function Lib_UIDropDownMenu_CreateFrames(level, index)
	while (level > LIB_UIDROPDOWNMENU_MAXLEVELS) do
		...
		local newList = CreateFrame("Button", "Lib_DropDownList" .. LIB_UIDROPDOWNMENU_MAXLEVELS, nil,
			"Lib_UIDropDownListTemplate");
		...
		for i = LIB_UIDROPDOWNMENU_MINBUTTONS + 1, LIB_UIDROPDOWNMENU_MAXBUTTONS do
			local newButton = CreateFrame("Button", "Lib_DropDownList" .. LIB_UIDROPDOWNMENU_MAXLEVELS .. "Button" .. i,
				newList, "Lib_UIDropDownMenuButtonTemplate");
			newButton:SetID(i);
		end
	end
	...
	while (index > LIB_UIDROPDOWNMENU_MAXBUTTONS) do
		...
		for i = 1, LIB_UIDROPDOWNMENU_MAXLEVELS do
			local newButton = CreateFrame("Button", "Lib_DropDownList" .. i .. "Button" .. LIB_UIDROPDOWNMENU_MAXBUTTONS,
				_G["Lib_DropDownList" .. i], "Lib_UIDropDownMenuButtonTemplate");
...
function Lib_UIDropDownMenu_AddButton(info, level)
	...
	if (info.hasColorSwatch) then
		_G["Lib_DropDownList" .. level .. "Button" .. index .. "ColorSwatch" .. "NormalTexture"]:SetVertexColor(info.r,
			info.g, info.b);
		button.r = info.r;
		button.g = info.g;
		button.b = info.b;
...
function Lib_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button,
								autoHideDelay)
	if (not level) then
		level = 1;
	end
...
function Lib_UIDropDownMenu_EnableDropDown(dropDown)
	...
	_G[dropDown:GetName() .. "Text"]:SetVertexColor(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g,
